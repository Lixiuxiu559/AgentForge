 ...
 *   不传 --lang 时按报告文件名自动探测。
 * 被 cleaner agent 调用; 只报告不改码, 超阈值以退出码 1 返回。
 */
"use strict";
const fs = require("fs");
...
function parseJavaClasses(pkgBody, pkgName, methods) {
  // 先剥离自闭合的 <class .../>（JaCoCo 对无方法的类——VO / 注解 / 空实现——输出这种形态）。
  // 它们没有配对的 </class>，若不剥离，下面的懒惰正则会把它当成开标签，一路吞到**下一个类**
  // 的 </class>，于是把后一个类的方法错记到它名下（实测全仓 716 个自闭合类、181 条方法归属出错）。
  const body = pkgBody.replace(/<class\b[^>]*\/>/g, "");
